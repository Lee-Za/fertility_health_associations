...
*****|
*****|original pairfam health scales: pcs mcs
*****|self calculated health scales:  hpcs hmcs
*****|
*****+--------------------------------------------------------------------------
...
*****+--------------------------------------------------------------------------
*****|
*****| Defining predictor parenthood vs. childlessness
*****|
*****+--------------------------------------------------------------------------
...
*****+--------------------------------------------------------------------------
*****|
*****| Defining predictor number of kids 
*****|
*****+--------------------------------------------------------------------------
...
*simple: number of kids alive, all anchors, automatically excluding dead children
tab nkidsalv
sum nkidsalv

*grouping number of kids, 0-4+
...
*Alternative: summarized cohabiting years as Indicator for Intensity of parentship
*used different dataset: biochild.dta 
*--> see dofile Dofilebiochild_livk.do
*Feed livk0_7 from extra dataset (master only 358, using only 4305, matched 1759)
merge 1:1 id using "$path_in/Data4_cohabiting_years.dta", gen (merge_livk)
tab merge_livk
...
*****+--------------------------------------------------------------------------
*****|
*****| Definin predictor age at first/last birth
*****|
*****+--------------------------------------------------------------------------
...
tab early_fbirth
*extra dummy for early first birth women: early_fbirth_w 15-22
gen early_fbirth_w = fbirth_dum1 if sex_gen == 2
*extra dummy for early first birth men: early_fbirth_m 13-26
...
numlabel lfem, add
tab fem
*male (just to get the right order when using by-option for creating graphs)
recode fem (0 = 1) (1 = 0), gen(male)
...
*****+--------------------------------------------------------------------------
*SMOKING
*Dummy: Currently Smoking (*Vergangenheit fehlt)
...
*DRINKING ALCOHOL
*drinking regularity: how often do you drink? (never - every day)
tab hlt13
...
*drinking intensity: last month: how often drank 5+ alcoholic beverages on one occasion
